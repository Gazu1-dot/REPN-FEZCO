...
import{EspCanvas,type ScenePlayback,type SceneStatus}from"./premium-hero";
...
export function SystemExplorer(){
 ...
 const[chapter,setChapter]=useState(0),[paused,setPaused]=useState(false),[inspect,setInspect]=useState(false),[reduced,setReduced]=useState(false),[status,setStatus]=useState<SceneStatus>("loading");
 ...
 return <section ref={section} className="exp-scroll" id="experience" aria-label="Explore the ESP system in 3D">
  <div className="exp-stage" ref={pin} data-chapter={chapter} data-inspect={inspect} data-status={status}>
   ...
   <div className="exp-view-tools"><button type="button" onClick={()=>setInspect(v=>!v)} disabled={status!=="ready"} aria-pressed={inspect}><Rotate3D size={15}/>{inspect?'Return to scroll':'Inspect in 3D'}</button>{!reduced&&<button type="button" onClick={()=>setPaused(v=>!v)} aria-pressed={paused} aria-label={paused?'Resume ambient animation':'Pause ambient animation'}>{paused?<Play size={14}/>:<Pause size={14}/>}</button>}</div>
